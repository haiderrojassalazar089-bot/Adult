"""
Responsabilidad:
Construir y serializar el pipeline de preprocesamiento
para el dataset Adult.

Incluye:
- Identificación automática de columnas
- Imputación de valores faltantes
- Escalamiento de variables numéricas
- Codificación robusta de variables categóricas
- Serialización del preprocesador como artefacto

Este módulo representa la etapa de FEATURE ENGINEERING
dentro del pipeline MLOps.
"""

import pandas as pd
from pathlib import Path
import joblib
import json
import logging
from datetime import datetime

from sklearn.pipeline import Pipeline
from sklearn.compose import ColumnTransformer
from sklearn.preprocessing import StandardScaler, OneHotEncoder
from sklearn.impute import SimpleImputer


# CONFIGURACIÓN DE LOGGING

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(levelname)s - %(message)s"
)


def identify_column_types(df: pd.DataFrame):
    """
    Identifica columnas numéricas y categóricas automáticamente.

    Parámetros
    ----------
    df : pd.DataFrame

    Retorna
    -------
    tuple(list, list)
        Lista de columnas numéricas y categóricas
    """

    num_cols = df.select_dtypes(include=["int64", "float64"]).columns.tolist()
    cat_cols = df.select_dtypes(include=["object", "category"]).columns.tolist()

    return num_cols, cat_cols


def build_preprocessor(df: pd.DataFrame) -> ColumnTransformer:
    """
    Construye un ColumnTransformer robusto.

    - Imputa valores faltantes
    - Escala numéricas
    - Codifica categóricas con manejo de categorías desconocidas

    Parámetros
    ----------
    df : pd.DataFrame

    Retorna
    -------
    ColumnTransformer
    """

    num_cols, cat_cols = identify_column_types(df)

    logging.info(f"Columnas numéricas detectadas: {num_cols}")
    logging.info(f"Columnas categóricas detectadas: {cat_cols}")

    # Pipeline numérico
    numeric_pipeline = Pipeline([
        ("imputer", SimpleImputer(strategy="median")),
        ("scaler", StandardScaler())
    ])

    # Pipeline categórico
    categorical_pipeline = Pipeline([
        ("imputer", SimpleImputer(strategy="most_frequent")),
        ("encoder", OneHotEncoder(
            handle_unknown="ignore",
            sparse_output=False
        ))
    ])

    preprocessor = ColumnTransformer([
        ("num", numeric_pipeline, num_cols),
        ("cat", categorical_pipeline, cat_cols)
    ])

    return preprocessor


def run_feature_engineering(
    input_path: str = "data/raw/features.parquet",
    artifacts_dir: str = "artifacts"
):
    """
    Ejecuta la construcción del preprocesador
    y lo serializa como artefacto.

    También genera metadata del proceso.
    """

    logging.info("Iniciando feature engineering...")

    df = pd.read_parquet(input_path)

    preprocessor = build_preprocessor(df)

    # Ajustar el preprocesador
    preprocessor.fit(df)

    # Crear directorio artefactos
    artifacts_path = Path(artifacts_dir)
    artifacts_path.mkdir(parents=True, exist_ok=True)

    # Serializar objeto
    joblib.dump(preprocessor, artifacts_path / "preprocessor.joblib")

    logging.info("Preprocesador serializado correctamente.")

    # Metadata del proceso
    metadata = {
        "n_rows": len(df),
        "n_original_features": df.shape[1],
        "timestamp_utc": datetime.utcnow().isoformat()
    }

    with open(artifacts_path / "feature_engineering_metadata.json", "w") as f:
        json.dump(metadata, f, indent=4)

    logging.info("Metadata de feature engineering generada.")


if __name__ == "__main__":
    run_feature_engineering()